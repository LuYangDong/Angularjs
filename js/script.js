...
// http://javascript-source.com
//***********************************************
function ws_parallax(j, f, a) {
	var c = jQuery;
	var e = c(this);
	var b = j.parallax || 0.25;
	var d = c("<div>").css({
		position: "absolute",
		top: 0,
		left: 0,
	...
	var g = h.clone().appendTo(d);
	this.go = function(k, q, o) {
		var r = c(f.get(q));
		r = {
			width: r.width(),
			height: r.height(),
		...
		o = o ? 1 : -1;
		var m = c(f.get(q)).clone().css(r).appendTo(h);
		var n = c(f.get(k)).clone().css(r).appendTo(g);
		var l = a.width() || j.width;
		var p = a.height() || j.height;
		wowAnimate(function(u) {
			u = c.easing.swing(u);
			var w = i(o * u * l),
				t = i(o * (-l + u * l)),
				s = i(-o * l * b * u),
			...
			if(j.support.transform) {
				h.css("transform", "translate3d(" + w + "px,0,0)");
				m.css("transform", "translate3d(" + s + "px,0,0)");
				g.css("transform", "translate3d(" + t + "px,0,0)");
				n.css("transform", "translate3d(" + v + "px,0,0)")
			} else {
				h.css("left", w);
				m.css("margin-left", s);
				g.css("left", t);
...
function ws_blast(p, i, l) {
	...
	function m(q) {
		...
		for(var t = 0; t < d * o; t++) {
			...
			e([b[t] = document.createElement("div"), k[t] = document.createElement("div")]).css({
				position: "absolute",
				overflow: "hidden"
			}).appendTo(j).append(e("<img>").css({
				position: "absolute"
			}))
		}
		k = e(k);
	...
	function n(s, t, r, q, v, y) {
		...
		e(s).each(function(E) {
			...
			e(this).find("img").css({
				left: -(u * D / d) + t.marginLeft,
				top: -(w * B / o) + t.marginTop,
				width: t.width,
				height: t.height
			});
			var z = {
...
function ws_blinds(l, k, a) {
	...
	this.go = function(q, o, n) {
		...
		var u = r.clone().css(s).appendTo(d);
		u.from = {
			left: 0
		};
		u.to = {
			left: (!n ? 1 : -1) * u.width() * 0.5
		};
		...
		wowAnimate(u, u.from, u.to, l.duration, l.duration * 0.1, "swing");
		for(var p = 0; p < e.length; p++) {
			var m = e[p];
			var v = g(k[q]).clone().css({
				position: "absolute",
				top: 0
			}).css(s).appendTo(m);
			v.from = {
				left: !n ? (-v.width()) : (v.width() - m.position().left)
			};
			v.to = {
			...
			wowAnimate(v, v.from, v.to, (l.duration / (e.length + 1)) * (n ? (e.length - p + 1) : (p + 2)), "swing", ((!n && p == e.length - 1 || n && !p) ? function() {
				j.trigger("effectEnd");
...
// http://javascript-source.com
//***********************************************
function ws_basic_linear(i, f, a) {
	var c = jQuery;
	var e = c(this);
	var h = c("<div>").addClass("ws_effect ws_basic_linear").css({
		position: "absolute",
		top: 0,
		left: 0,
		width: "100%",
	...
	var g = c("<div>").css({
		...
		d = g.clone();
	b.append(g, d);
	this.go = function(j, m, l) {
		b.stop(1, 1);
		if(l == undefined) {
			l = (!!((j - m + 1) % f.length) ^ i.revers ? "left" : "right")
		} else {
			l = l ? "left" : "right"
		}
		var n = c(f[m]);
		...
		n.clone().css(k).appendTo(g).css(l, 0);
		c(f[j]).clone().css(k).appendTo(d).show();
		if(l == "right") {
			g.css("left", "50%");
			d.css("left", 0)
		} else {
		...
		wowAnimate(b.css({
			...
			right: "auto",
			top: 0
		}).css(l, 0).show(), p, o, i.duration, "easeInOutExpo", function() {
			e.trigger("effectEnd");
...
jQuery("#wowslider-container1").wowSlider({
	effect: "lines,bubbles,dribbles,glass_parallax,parallax,brick,collage,seven,kenburns,cube,blur,book,rotate,domino,slices,blast,blinds,basic,basic_linear,fade,fly,flip,page,stack,stack_vertical",
	prev: "",
	next: "",
	duration: 20 * 100,
	delay: 20 * 100,
